...
test.describe("投票記録のライフサイクル", () => {
  ...
  test("投票記録を登録し、詳細を確認後に削除できること", async ({ page }) => {
    // 1. 登録ページへ遷移
    await page.getByRole("link", { name: "投票記録登録" }).click();
    ...
    await page.getByRole("option").first().click();
    await page.getByLabel("候補者名（任意）").fill("テスト 太郎");
    await page.getByLabel("メモ（任意）").fill("Playwrightによる自動テストの記録です。");
    ...
    await expect(page.getByText("テスト自動選挙 2026")).toBeVisible();
    ...
    await page.waitForURL(url => url.pathname === "/history", { timeout: 10000 });
    await page.reload(); 
    await expect(page.getByText("過去の投票履歴").first()).toBeVisible();
